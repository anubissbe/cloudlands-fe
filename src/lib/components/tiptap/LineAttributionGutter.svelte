...
<div class="absolute left-0 top-0 w-6 h-full pointer-events-none z-10">
  {#each spans as span (span.positions.join(','))}
    ...
    <div
      class="absolute -left-40 w-44 pointer-events-auto group/span"
    ...
    >
      <div
        class="absolute w-1 right-4 h-full rounded-sm bg-muted transition-[width] duration-200 group-hover/span:w-1.5"
        style:opacity={span.opacity}
      ></div>
      <!-- Avatar and label for spans (shown on hover) -->
      <div
        class="flex flex-row items-center gap-1 leading-none absolute right-4 pr-4 pl-0.5 rounded-md text-xs whitespace-nowrap text-muted-foreground opacity-0 group-hover/span:opacity-100 transition-opacity duration-200"
