...
describe('window close bridge (guest-offline overlay "Close window")', () => {
  beforeEach(() => {
    resetMockIpcRouter();
  });
  ...
  it('forwards window:close to window.electronAPI.invoke when bridged', async () => {
    const invokeSpy = vi.fn(async () => ({ success: true }));
    (window as any).electronAPI = { ...(originalElectronAPI || {}), invoke: invokeSpy };
    registerWindowCloseBridge();
    ...
    expect(invokeSpy).toHaveBeenCalledOnce();
    expect(invokeSpy).toHaveBeenCalledWith(IPC_CHANNELS.WINDOW.CLOSE, undefined);
  });
