...
// The search saga is started against the REAL configured store, so dispatching
...
describe('githubRepoSearchSaga (fake seam, real store)', () => {
  let stopSaga: () => void;
  let disposeStore: () => void;

  beforeAll(() => {
    disposeStore = appStore.init();
    stopSaga = appStore.runSaga(githubRepoSearchSaga);
  });
  afterAll(() => {
    stopSaga();
    disposeStore();
  ...
  // failure path the saga must surface.
  ...
  // takeLatest cancellation is request-instance based, not query-based: two searches for the SAME string
