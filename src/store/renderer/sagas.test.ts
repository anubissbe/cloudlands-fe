...
describe('renderer app saga registry', () => {
  ...
  it('registers every audited root saga exactly once', () => {
    ...
    expect(names).toEqual(expectedSagaNames);
    expect(new Set(sagas).size).toBe(expectedSagaNames.length);
  });
  ...
  it('returns one cancellation handler per registered saga', () => {
    ...
    expect(store.runSaga).toHaveBeenCalledTimes(expectedSagaNames.length);
    expect(store.runSaga.mock.calls.map(([saga]) => saga)).toEqual(sagas);
    expect(handlers).toEqual(Array(expectedSagaNames.length).fill(cancel));
  });
